...
use crate::{account::{crud_utils::query_user_in_stake_accounts, guard_keys::get_recovery_unstake_penalty_guard_key, stable_structures::{StakingAccount, StakingAccountRecoverableError, StakingAccountStatus}, transport_structures::StakingAccountVo, STAKING_RECOVERABLE_ERROR_ACCOUNT_INDEX_MAP}, event_log::{stake_and_unstake_events::save_unstake_event, transfer_events::{save_dissolve_pay_center_receive_fail_event, save_unstake_penalty_pay_center_ok_event, save_unstake_penalty_pay_center_start_event, save_unstake_penalty_transfer_fail_event, save_unstake_penalty_transfer_ok_event, save_unstake_penalty_transfer_start_event}}, on_chain::transfer::transfer_from_staking_pool_to_pay_center, parallel_guard::EntryGuard, pool::stable_structures::StakingPool, system_configs::get_exteral_canister_id};
...
pub async fn recover_unstake_penalty_onchain_error(account: &StakingAccount) -> Result<StakingAccountVo, String> {
  ...
  if let Some(StakingAccountRecoverableError::EarlyUnstakePenaltyOnChainFailed(unstake_tx_id, unstake_time, penalty_amount)) = account.recoverable_error {
    ...
    let stake_response = match pay_center.receive_early_unstake_penalty(
      ...
      Err(e) => {
        ...
        account.stable_to_recoverable_error(StakingAccountRecoverableError::EarlyUnstakePenaltyPayCenterFailed(unstake_tx_id, penalty_onchain_tx_id, unstake_time, penalty_amount));
...
pub async fn recover_unstake_penalty_pay_center_error(account: &StakingAccount) -> Result<StakingAccountVo, String> {
  ...
  if let Some(StakingAccountRecoverableError::EarlyUnstakePenaltyPayCenterFailed(unstake_tx_id,  penalty_onchain_tx_id,unstake_time, penalty_amount)) = account.recoverable_error {
    // Verify the status of the staked account
    if account.get_status() != StakingAccountStatus::InStake {
      return Err("The staking account is not in stake".to_string());
    }

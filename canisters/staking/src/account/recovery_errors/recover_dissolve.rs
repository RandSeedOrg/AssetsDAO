...
use crate::{account::{guard_keys::get_recovery_dissolve_guard_key, stable_structures::{StakingAccount, StakingAccountStatus}, transport_structures::StakingAccountVo, STAKING_RECOVERABLE_ERROR_ACCOUNT_INDEX_MAP}, event_log::{stake_and_unstake_events::save_dissolve_event, transfer_events::{save_dissolve_pay_center_receive_fail_event, save_dissolve_pay_center_receive_ok_event, save_dissolve_pay_center_receive_start_event, save_stake_pay_center_transfer_fail_event}}, parallel_guard::EntryGuard, system_configs::get_exteral_canister_id};
...
pub async fn recover_dissolve_error(account: &StakingAccount, dissolve_tx_id: BlockIndex) -> Result<StakingAccountVo, String> {
  ...
  let dissolve_response = match pay_center.dissolve(
    caller,

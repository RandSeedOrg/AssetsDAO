...
pub async fn add_staker_badge(user_principal: String, account_id: StakingAccountId) -> Result<(), String> {
  ...
  messenger.update_user_badges(user_principal, STAKER_BADGE, false, Some(vec![
    BadgeWithProps {
      badge: STAKER_BADGE,
    ...
    format!("Failed to add staker badge: {:?}", e)
  })?;
...
pub async fn remove_staker_badge(user_principal: String) -> Result<(), String> {
  ...
  messenger.update_user_badges(user_principal, STAKER_BADGE, true, None).await.map_err(|e| {
    format!("Failed to remove staker badge: {:?}", e)
  })?;

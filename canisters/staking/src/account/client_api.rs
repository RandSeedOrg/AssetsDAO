...
use types::{date::YearMonthDay, entities::add_indexed_id, sys::ExteralCanisterLabels};

use crate::{account::{crud_utils::delete_staking_account, stable_structures::StakingAccountRecoverableError}, event_log::{stake_and_unstake_events::{save_dissolve_event, save_stake_event, save_unstake_event}, staking_account_events::save_create_staking_account_event_log, transfer_events::{save_dissolve_pay_center_receive_fail_event, save_dissolve_pay_center_receive_ok_event, save_dissolve_pay_center_receive_start_event, save_dissolve_pay_center_transfer_ok_event, save_dissolve_pay_center_transfer_start_event, save_stake_pay_center_transfer_fail_event, save_stake_pay_center_transfer_ok_event, save_stake_pay_center_transfer_start_event, save_stake_transfer_fail_event, save_stake_transfer_ok_event, save_stake_transfer_start_event, save_unstake_penalty_pay_center_ok_event, save_unstake_penalty_pay_center_start_event, save_unstake_penalty_transfer_fail_event, save_unstake_penalty_transfer_ok_event, save_unstake_penalty_transfer_start_event, save_unstake_transfer_fail_event, save_unstake_transfer_ok_event, save_unstake_transfer_start_event}}, on_chain::transfer::{transfer_from_staking_account_to_pay_center, transfer_from_staking_account_to_staking_pool, transfer_from_staking_pool_to_pay_center, transfer_from_staking_pool_to_staking_account}, parallel_guard::EntryGuard, pool::{crud_utils::query_staking_pool_by_id, stable_structures::StakingPool}, system_configs::get_exteral_canister_id, StakingAccountId, StakingPoolId};

use super::{client_transport_structures::{EarlyUnstakePreCheckVo, StakeDto}, crud_utils::{query_current_user_in_stake_accounts, query_current_user_staking_accounts, save_stake_account_to_stable_memory}, guard_keys::{get_dissolve_guard_key, get_stake_guard_key, get_unstake_guard_key}, recovery_errors::{recover_dissolve::recover_dissolve_error, recover_early_unstake::{recover_unstake_penalty_onchain_error, recover_unstake_penalty_pay_center_error}}, stable_structures::{StakingAccount, StakingAccountStatus}, transport_structures::StakingAccountVo, STAKING_UNSTAKE_ON_DAY_ACCOUNT_INDEX_MAP};
...
async fn stake(dto: StakeDto) -> Result<StakingAccountVo, String> {
  ...
  let StakeDto { pool_id, staking_amount, staking_days } = dto;
  ...
  let stake_response = match pay_center.stake(
    caller,
  ...
  let staking_account_to_pool_tx_id = match transfer_from_staking_account_to_staking_pool(account.get_id(), staking_pool.get_id(), account.get_staked_amount()).await {
    Ok(tx_id) => {
    ...
    Err(e) => {
      ...
      account.stable_to_recoverable_error(StakingAccountRecoverableError::StakeTransferToPoolFailed(stake_pay_center_onchain_tx_id, stake_pay_center_tx_id));
  ...
  account.change_to_in_stake(
    stake_pay_center_onchain_tx_id,
    stake_pay_center_tx_id,
    staking_account_to_pool_tx_id,
  );
  save_stake_account_to_stable_memory(&account)?;
...
async fn early_unstake(account_id: StakingAccountId) -> Result<StakingAccountVo, String> {
  ...
  if now < account.get_can_early_unstake_time() {
    return Err(format!("The staking account cannot be released within {} days.", account.get_min_early_unstake_days()));
  }
  ...
  let unstake_tx_id = if released_amount > 0 { // Unstake金额大于0，On-chain transfer is required
  ...
  let (penalty_onchain_tx_id, penalty_pay_center_tx_id) = if penalty_amount > 0 {
    ...
    let pay_center = common_canisters::pay_center::Service(pay_center_canister_id);
    let stake_response = match pay_center.receive_early_unstake_penalty(
      caller,
      penalty_amount,
      account.get_pool_id(),
      account.get_id(),
      ...
      Err(e) => {
        ...
        account.stable_to_recoverable_error(StakingAccountRecoverableError::EarlyUnstakePenaltyPayCenterFailed(unstake_tx_id, penalty_onchain_tx_id, now, penalty_amount));
  ...
  // Update the status of the staked account，and save to stable memory
  let updated_account = match account.change_to_un_stake(
    unstake_tx_id, 
    released_amount,
    penalty_amount, 
    now,
    penalty_onchain_tx_id,
    penalty_pay_center_tx_id,
  ) {
    Ok(account) => account,
    Err(e) => {
...
async fn dissolve(account_id: StakingAccountId) -> Result<StakingAccountVo, String> {
  ...
  let (dissolve_tx_id, pay_center_tx_id) = if account.get_released_amount() == 0 {  // Unstake金额为0，No on-chain transfer is required
    ic_cdk::println!("The released amount is 0, no need to transfer on-chain");
    (0, 0)
  } else { // Unstake amount great than 0，On-chain transfer is required
    ...
    let stake_response = match pay_center.dissolve(
      caller,

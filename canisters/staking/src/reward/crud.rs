...
use super::{transport_structures::{StakingRewardPageRequest, StakingRewardPageResponse, StakingRewardQueryParams, StakingRewardVo}, STAKING_ACCOUNT_REWARD_INDEX_MAP, STAKING_POOL_REWARD_INDEX_MAP, STAKING_REWARD_MAP, STAKING_USER_REWARD_INDEX_MAP};
...
fn query_staking_rewords(request: StakingRewardPageRequest) -> StakingRewardPageResponse {
  ...
  let indexed_records = STAKING_REWARD_MAP.with(|map| {
    ...
    if user_id.len() == 63 {
      STAKING_USER_REWARD_INDEX_MAP.with(|m| get_indexed_ids(m, &user_id))
        .iter()
        .filter_map(|id| map.get(id))
        .collect::<Vec<_>>()
    } else if account_id > 0 { // If the account is passedID，Get the corresponding one from the account index queryID
      STAKING_ACCOUNT_REWARD_INDEX_MAP.with(|m| get_indexed_ids(m, &account_id))
        .iter()
        .filter_map(|id| map.get(id))
        .collect::<Vec<_>>()
    } else if pool_id > 0 { // If the stake pool is passedID，Get the corresponding one from the staking pool index queryID
      STAKING_POOL_REWARD_INDEX_MAP.with(|m| get_indexed_ids(m, &pool_id))
        .iter()
        .filter_map(|id| map.get(id))
        .collect::<Vec<_>>()
    } else { // If no indexes are passed in，Query from all reward data
      map.iter()
        .map(|(_, record)| record)
        .collect::<Vec<_>>()
    }

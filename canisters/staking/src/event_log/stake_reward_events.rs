...
pub fn save_reward_distribute_event(reward: &StakingReward, account: &StakingAccount) {
  EventLog::new(EventType::DistributeReward(reward.clone(), account.clone())).save_to_stable_memory()
}

/// Rewards Incident
pub fn save_reward_received_event(reward: &StakingReward) {
  EventLog::new(EventType::RewardReceived(reward.clone())).save_to_stable_memory()
}

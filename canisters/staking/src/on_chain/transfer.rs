...
pub async fn transfer_from_staking_pool_to_staking_account(pool_id: StakingPoolId, account_id: StakingAccountId, amount: E8S) -> Result<BlockIndex, String> {
  ...
  transfer(&from_account, &to_account, amount + 10_000, Memo(TRANSFER_SCENE_UNSTAKE)).await // Use a valid u64 value for Memo
}

/// Transfer from staking account to staking pool (Stake)
pub async fn transfer_from_staking_account_to_staking_pool(account_id: StakingAccountId, pool_id: StakingPoolId, amount: E8S) -> Result<BlockIndex, String> {
  // Transfer out of the account
...
pub async fn transfer_from_staking_account_to_pay_center(from_account_id: StakingAccountId, amount: E8S) -> Result<BlockIndex, String> {
  ...
  let (pay_center_address,) = pay_center.get_address().await.map_err(|e| format!("Failed to obtain payment center address: {:?}", e))?;
